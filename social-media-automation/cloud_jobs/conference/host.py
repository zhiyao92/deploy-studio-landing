"""Short-lived, MIME-correct public Conference media in Cloud Storage."""
import os
from urllib.parse import quote
import requests
from google.cloud import storage

FOLDER='conference/';PREFIX='conference-media--';LIMIT=350*1024*1024
...
class MediaHost:
    ...
    def upload(self,path,run_id,index):
        ...
        if size>25*1024*1024 or len(objects)>=80 or sum(o.size or 0 for o in objects)+size>LIMIT:raise RuntimeError('Media retention budget reached')
        name=FOLDER+f'{PREFIX}{run_id}--{index}{path.suffix}'
        kind='video/mp4' if path.suffix.lower()=='.mp4' else 'image/png'
        blob=self.bucket.blob(name);blob.cache_control='public,max-age=604800';blob.upload_from_filename(str(path),content_type=kind)
        ...
        with requests.get(url,stream=True,timeout=30) as check:
            ...
            if check.status_code!=200 or actual!=kind or not next(check.iter_content(1024),b''):raise RuntimeError('Public media is not Meta-compatible')
        return url
